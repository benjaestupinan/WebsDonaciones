const validadorNombreDonante = (name) => {
  if(!name) return false;
  let lengthValid = name.trim().length >= 3 && name.trim().length <= 80;
  return lengthValid;
}

const validadorMail = (email) => {
...
const validadorTelefono = (phoneNumber) => {
...
const validadorNombreDispositivo = (nombreDispositivo) => {
    return nombreDispositivo.length >= 3 && nombreDispositivo.length <= 80
}
...
const validadorAñosDeUso = (años) => {
    const a = parseInt(años)
    return !isNaN(a) && a > 0 && a < 100
}

const validadorFuncionamiento = (funcionamiento) => {
    return funcionamiento != "nada"
}

const validadorFotos = (files) => {
...
regiones.regiones.forEach(region => {
    ...
    regionOption.textContent = region.nombre
    regionOption.value = region.nombre
    regionSelect.appendChild(regionOption)
})

let comunasSelect = document.getElementById("comuna")

const refreshComunas = () => {
    comunasSelect.innerHTML = ''
    regiones.regiones.forEach(region => {
        if (region.nombre != regionSelect.value) {
            
        } else {
            region.comunas.forEach( comuna => {    
                const comunaOption = document.createElement("option")
                comunaOption.textContent = comuna.nombre
                comunasSelect.appendChild(comunaOption)
            }
            )
        }
    })
...
function eliminarDispositivo(id) {
    let disp = document.getElementById(`aloalo${id}`)
    disp.remove()
    disp_count--;
}

let agregarButton = document.getElementById("agregar-dispositivo")

let submitBtn = document.getElementById("publicar-donacion")

let body = document.getElementById("body")

const agregarDispositivo2 = () => {
    let disp_count_input = document.getElementById("disp_count")
    let dispositivosDiv = document.getElementById("parent")
    let agregarDispDiv = document.getElementById(`aloalo${disp_count}`)
    disp_count++
    disp_count_input.value = disp_count+1
    let nuevoDiv = agregarDispDiv.cloneNode(true)
    nuevoDiv.id = `aloalo${disp_count}`
    nuevoDiv.classList = ["aaa"]
    nuevoDiv.classList.remove("borrado")
    
    dispositivosDiv.appendChild(nuevoDiv)
    
    let btnBorrarNuevoDisp = nuevoDiv.querySelector(`#btn-borrar${disp_count-1}`)
    btnBorrarNuevoDisp.id = `btn-borrar${disp_count}`
    btnBorrarNuevoDisp.name = disp_count
    btnBorrarNuevoDisp.classList = ["buttons"]
    btnBorrarNuevoDisp.onclick = () => {
        console.log(`eliminado id ${btnBorrarNuevoDisp.name}`)
        eliminarDispositivo(btnBorrarNuevoDisp.name)
    }

    let nombreNuevoDisp = nuevoDiv.querySelector(`#nombre-disp${disp_count-1}`)
    nombreNuevoDisp.id = `nombre-disp${disp_count}`
    nombreNuevoDisp.name = `nombre-disp${disp_count}`
    nombreNuevoDisp.classList = ["smooth"]

    let descripNuevoDisp = nuevoDiv.querySelector(`#description${disp_count-1}`)
    descripNuevoDisp.id = `description${disp_count}`
    descripNuevoDisp.name = `description${disp_count}`
    descripNuevoDisp.classList = ["smooth"]

    let tipoNuevoDisp = nuevoDiv.querySelector(`#tipo${disp_count-1}`)
    tipoNuevoDisp.id = `tipo${disp_count}`
    tipoNuevoDisp.name = `tipo${disp_count}`
    tipoNuevoDisp.classList = ["smooth"]

    let añosNuevoDisp = nuevoDiv.querySelector(`#años-de-uso${disp_count-1}`)
    añosNuevoDisp.id = `años-de-uso${disp_count}`
    añosNuevoDisp.name = `años-de-uso${disp_count}`
    añosNuevoDisp.classList = ["smooth"]

    let estadoNuevoDisp = nuevoDiv.querySelector(`#estado-funcionamiento${disp_count-1}`)
    estadoNuevoDisp.id = `estado-funcionamiento${disp_count}`
    estadoNuevoDisp.name = `estado-funcionamiento${disp_count}`
    estadoNuevoDisp.classList = ["smooth"]

    let fotoNuevoDisp = nuevoDiv.querySelector(`#foto-producto${disp_count-1}`)
    fotoNuevoDisp.id = `foto-producto${disp_count}`
    fotoNuevoDisp.name = `foto-producto${disp_count}`
    fotoNuevoDisp.classList = ["smooth"]


    agregarDispDiv = nuevoDiv
}

const preguntarComfirmaDonacion = () => {
    ...
    parraf.innerText = "¿Confirma que desea publicar esta donación?"
    parraf.className = "parraf"

    let buttonsDiv = document.createElement("div")
    buttonsDiv.className = "btnsDiv"

    let btnYes = document.createElement("button")
    btnYes.innerText = "Sí, confirmo"
    btnYes.className = "buttons"
    btnYes.type = "submit"

    let btnNo = document.createElement("button")
    btnNo.innerText = "No, quiero volver al formulario"
    btnNo.className = "buttons"
    btnNo.onclick = () => {
        modal.remove()
    }
...
const validarFormulario = () => {
    let errCont = document.getElementById("erroresContacto")
    errCont.innerHTML = ""
    let cumple = true
    //validar donante
    if (validadorNombreDonante(document.getElementById("nombre").value)) {
        document.getElementById("nombre").className = "correct"
    } else {
        errCont.innerHTML += `<li>El nombre no es valido</li>`
        cumple = false
        document.getElementById("nombre").className = "wrong"
    }
    if (validadorMail(document.getElementById("mail").value)) {
        document.getElementById("mail").className = "correct"
        // console.log("EEEEEEEEEEEEEEEEEEEEEE")
    } else {
        errCont.innerHTML += `<li>El mail no es valido</li>`
        cumple = false
        // console.log("BUUUUUUUUUUUUUUUUUUU")
        document.getElementById("mail").className = "wrong"
    }
    if (validadorTelefono(document.getElementById("celular").value)) {
        document.getElementById("celular").className = "correct"
    } else {
        errCont.innerHTML += `<li>El celular no es valido</li>`
        cumple = false
        document.getElementById("celular").className = "wrong"
    }
    if (validadorRegion(document.getElementById("region").value)) {
        document.getElementById("region").className = "correct"
    } else {
        errCont.innerHTML += `<li>La region no es valida</li>`
        cumple = false
        document.getElementById("region").className = "wrong"      
    }
    if (validadorComuna(document.getElementById("comuna").value)) {
        document.getElementById("comuna").className = "correct"
    } else {
        errCont.innerHTML += `<li>La comuna no es valida</li>`
        cumple = false
        document.getElementById("comuna").className = "wrong"
    }


    //validar dispositivos
    let errDisp = document.getElementById("erroresDisp")
    errDisp.innerHTML = ""
    for (let index = 0; index <= disp_count; index++) {
        if (validadorNombreDispositivo(document.getElementById(`nombre-disp${index}`).value)) {
            document.getElementById(`nombre-disp${index}`).classList.remove("wrong")
            document.getElementById(`nombre-disp${index}`).classList.add("correct", "smooth")
        } else {
            errDisp.innerHTML += `<li>El nombre del dispositivo N°${index+1} no es valido</li>`
            cumple = false
            document.getElementById(`nombre-disp${index}`).classList.remove("correct")
            document.getElementById(`nombre-disp${index}`).classList.add("wrong", "smooth")
        }
        if (validadorTipo(document.getElementById(`tipo${index}`).value)) {
            document.getElementById(`tipo${index}`).classList.remove("wrong")
            document.getElementById(`tipo${index}`).classList.add("correct", "smooth")
        } else {
            errDisp.innerHTML += `<li>El tipo del dispositivo N°${index+1} no es valido</li>`
            cumple = false
            document.getElementById(`tipo${index}`).classList.remove("correct")
            document.getElementById(`tipo${index}`).classList.add("wrong", "smooth")
        }
        if (validadorAñosDeUso(document.getElementById(`años-de-uso${index}`).value)) {
            document.getElementById(`años-de-uso${index}`).classList.remove("wrong")
            document.getElementById(`años-de-uso${index}`).classList.add("correct", "smooth")
        } else {
            errDisp.innerHTML += `<li>Los años de uso del dispositivo N°${index+1} no son validos</li>`
            cumple = false
            document.getElementById(`años-de-uso${index}`).classList.remove("correct")
            document.getElementById(`años-de-uso${index}`).classList.add("wrong", "smooth")
        }
        if (validadorFuncionamiento(document.getElementById(`estado-funcionamiento${index}`).value)) {
            console.log(document.getElementById(`estado-funcionamiento${index}`).value)
            document.getElementById(`estado-funcionamiento${index}`).classList.remove("wrong")
            document.getElementById(`estado-funcionamiento${index}`).classList.add("correct", "smooth")
        } else {
            errDisp.innerHTML += `<li>El estado del dispositivo N°${index+1} no es valido</li>`
            cumple = false
            document.getElementById(`estado-funcionamiento${index}`).classList.remove("correct")
            document.getElementById(`estado-funcionamiento${index}`).classList.add("wrong", "smooth")
        }
        if (validadorFotos(document.getElementById(`foto-producto${index}`).files)) {
            document.getElementById(`foto-producto${index}`).classList.remove("wrong")
            document.getElementById(`foto-producto${index}`).classList.add("correct", "smooth")
        } else {
            errDisp.innerHTML += `<li>Los archivos del dispositivo N°${index+1} no son validos</li>`
            cumple = false
            document.getElementById(`foto-producto${index}`).classList.remove("correct")
            document.getElementById(`foto-producto${index}`).classList.add("wrong", "smooth")
        }
        if(!cumple) {
            errDisp.innerHTML += '<div class="divider"></div>'
        }
    }
    if (cumple) {
        preguntarComfirmaDonacion()
    }
}


agregarButton.addEventListener("click",agregarDispositivo2)
regionSelect.addEventListener("change", refreshComunas)
